...
python3 $WORK_DIR/src_watermark/xsir/generate_embeddings.py \
	...
	--output_path $DATA_DIR/embedding/train_embeddings.txt \
	--model_path paraphrase-multilingual-mpnet-base-v2 \
	--size 2000

python $WORK_DIR/src_watermark/xsir/train_watermark_model.py \
    --input_path $DATA_DIR/embedding/train_embeddings.txt \
    --output_model $DATA_DIR/model/transform_model_x-sbert.pth \
    --input_dim 768

python3 $WORK_DIR/src_watermark/xsir/analysis_transform_model.py \
	--embedding_file $DATA_DIR/embedding/train_embeddings.txt \
	--input_dim 768 \
	--checkpoint $DATA_DIR/model/transform_model_x-sbert.pth \

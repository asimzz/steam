    ...
    for SEED in "${SEEDS[@]}"; do
        for WATERMARK_METHOD in "${WATERMARK_METHODS[@]}"; do
            ...
            for TGT_LANG in "${TGT_LANGS[@]}"; do
                ...
                evaluate_detection "$WATERMARK_DIR/mc4.en.hum.z_score.jsonl" "$WATERMARK_DIR/mc4.$ORG_LANG-$TGT_LANG-cwra.mod.z_score.jsonl"

                echo "======================================="

                echo "$MODEL_NAME $WATERMARK_METHOD With CWRA Attack ($PVT_LANG -> $ORG_LANG)"
                evaluate_detection "$WATERMARK_DIR/mc4.en.hum.z_score.jsonl" "$WATERMARK_DIR/mc4.$PVT_LANG-$ORG_LANG-cwra.mod.z_score.jsonl"
